...
class IndexDB {
  ...
  openDB() {
    const idb =
      window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB; //add if (idb)
  ...
  addWarehousesToBD(data) {
    ...
    request.onupgradeneeded = function (event) {
      ...
      if (!db.objectStoreNames.contains("warehouses")) {
        const objectStore = db.createObjectStore("warehouses", {
          keyPath: "SiteKey",
        });
        objectStore.createIndex("SettlementRef", "SettlementRef", {
          unique: false,
        });
